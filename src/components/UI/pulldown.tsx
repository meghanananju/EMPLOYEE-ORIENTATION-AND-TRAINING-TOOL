...
const Select: React.FC<SelectProps> = ({ options, selectType = 'single', placeholder = 'Select Value', value, label, required, errorMsg, disabled }) => {
    ...
    if (selectType === 'single') {
      ...
      setSelected((prev) => {
        ...
        if (prevArray.some((item) => item.id === option.id)) {
          return prevArray.filter((item) => item.id !== option.id);
        } else {
  ...
  const handleSelectAll = () => {
    if (disabled) return;
    if (Array.isArray(selected) && selected.filter((item) => !Array.isArray(item)).length === options.length) {
      setSelected([]);
    } else {
      setSelected([...options]);
    }
  ...
  const isSelected = (option: dataProps) =>
    selectType === 'single'
      ...
      : (Array.isArray(selected) && selected.some((item) => item.id === option.id));
  ...
  const displayValue = () => {
    ...
    const selectedArray = Array.isArray(selected) ? selected.filter((item) => !Array.isArray(item)) : [];
  ...
  const isAllSelected = Array.isArray(selected) && selected.filter((item) => !Array.isArray(item)).length === options.length;

  return (
    <div> 
    <Listbox value={selected} onChange={handleSelect} multiple={selectType === 'multi'} disabled={disabled}>
      {label && (
      ...
      <div className="relative mt-2">
        ...
        <ListboxOptions className="absolute z-10 mt-1 max-h-60 w-full overflow-auto bg-gray-50 shadow-xl ring-1 rounded-lg ring-black/5 focus:outline-none sm:text-sm">
          <div className="p-2 sticky top-0 bg-white z-10 border-b border-gray-200">
            ...
            />
            {selectType === 'multi' && (
              <div className="flex items-center gap-2 mt-2 cursor-pointer" onClick={handleSelectAll}>
                <input
                  type="checkbox"
          ...
          {filteredOptions.map((option) => (
            <ListboxOption 
              key={option.id} 
              value={option} 
              className={`select-none p-2 hover:bg-blue-400   border-gray-400 last:border-none font-serif cursor-pointer ${isSelected(option) ? 'bg-blue-200' : ''}`}
      ...
      {errorMsg && <p className="text-red-500 text-sm font-serif mt-1">{errorMsg}</p>}
    </Listbox>
      </div>
  );
};
